...
// supprimer une réservation
    ...
    routes.get('/getByStation/:_id', async ( req, res)=>{
        ...
        //get reservation by id station
        routes.get('/getByS/:Station', async ( req, res)=>{

            try {
                
            const oneComt = await Reservation.find({Station:req.params.Station , etat:'en attente'}).populate("Station");
            res.status(201).json(oneComt);
...
/*
        routes.put('/updateEtatfalse/:id', async (req, res) =>{
            try {
               const id = req.params.id
               let mdd = {
                   etat:false
...
routes.put('/updateEtat1/:id', async (req, res) =>{
    try {
       const id = req.params.id
       let mdd = {
           etat:'refuser'
